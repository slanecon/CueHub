...
- `npm start` — runs server.ts via ts-node on http://localhost:3000
- `npm run build` — bundles src/app.ts → public/app.js via esbuild
- `npm run build:copy` — build + copy public/app.js to desktop/CueHub/Resources/Web/app.js
- `npm run build:server` — compile server.ts → dist/server.js via tsc
- `npm run typecheck` — type-check frontend and server (no emit)
- `npm install` — install all dependencies
- No test runner or linter configured

### macOS Desktop App (desktop/)
- `cd desktop && xcodegen generate` — regenerate Xcode project from project.yml
...
### Web App (server.ts + src/app.ts + public/)
Express.js server with SQLite (better-sqlite3, WAL mode). TypeScript frontend compiled by esbuild to public/app.js — no frameworks. Real-time updates via Server-Sent Events (SSE). The PUT endpoint implements 3-way field-level merge: clients send `baseCue` (state when form was loaded) alongside edits, and the server auto-merges non-conflicting field changes or returns 409 with conflict details. Shared types live in src/types.ts.
...
## Code Conventions
- **TypeScript**: Strict mode (`strict: true`). Frontend bundled with esbuild; server run with ts-node. Shared types in `src/types.ts`. Generic `api<T>()` helper for typed API calls. `ConflictError` extends `Error` for typed 409 handling.
...
- Web assets (index.html, app.js, style.css) are in public/ for the server and copied to desktop/CueHub/Resources/Web/ for the desktop app bundle